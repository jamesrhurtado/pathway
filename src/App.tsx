...
function App() {
  ...
  const bridge = useMemo(() => ({ state, stagedPlan: packet, approved: packet?.status === 'approved', stagePacket, stageSchedule, stageStaff, stageAnnouncement, reviewPlan, revisePlan, publishPlan }), [packet, publishPlan, revisePlan, reviewPlan, stageAnnouncement, stagePacket, stageSchedule, stageStaff, state])
  useEffect(() => {
  ...
  const reset = () => { setState(initialEvent); setPacket(undefined); setSelectedIncidentId('room-b-capacity'); setLocked(true); setTools([]); notify('Demo state reset') }
  ...
  return <div className="app-shell">
    ...
    <main className="main-wrap">
      <section className="hero-grid">
        <div className="hero-copy"><div className="eyebrow"><span className="eyebrow-line" />LIVE EVENT TWIN <span className="mono">/ {state.event.date}</span></div><h1>Keep the room moving.<br /><em>Leave the final call human.</em></h1><p>Backstage gives an agent enough context to diagnose a live event, stage a coordinated response, and hand you a clear decision packet before anything changes.</p><div className="hero-command"><div className="command-icon"><Bot size={18} /></div><div className="command-text"><span className="command-label">TRY THE HERO SCENARIO</span><span>“Room B is over capacity and 17 builders are blocked on auth. Find the least disruptive response, but don’t publish.”</span></div><button className="primary-button" onClick={stageHero}><Play size={15} fill="currentColor" /> Run rehearsal</button></div></div>
