...
function App() {
  ...
  const stagePacket = useCallback((input?: { incidentId?: string }) => {
    ...
    const incidentId = input?.incidentId ?? 'room-b-capacity'
    if (!actionableIncidentIds.includes(incidentId as (typeof actionableIncidentIds)[number])) return { ok: false, error: `Incident ${incidentId} is not actionable in this rehearsal.`, recovery: `Use one of: ${actionableIncidentIds.join(', ')}.` }
